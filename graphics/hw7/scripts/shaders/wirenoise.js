...
CG.Shader.wirenoise = (function() {
	...
	varying vec4 vXYZW;" + noise + "\
	vec4 vec4Noise(vec4 p){\
		float x = turbulence(p.xyz);\
		float y = turbulence(p.yzx);\
		return vec4(x,y,0,0.0);\
	}\
	void main(void) {\
		vec4 nPosition = uPMatrix * uMVMatrix * vec4(aVertexPosition, 1.0);\
		gl_Position = nPosition + 0.1*vec4Noise(nPosition*2.0);\
		vXYZW = gl_Position;\
	}";
